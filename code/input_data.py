...
class InputData:
    ...
    def __init__(self, ExcelPath, demands='Demand',solar='Solar Profile', tech='Energy Converters', stg='Storage', gen='General', ec='Energy Carriers', net = 'Network', imp = 'Imports', exp = 'Exports', impHr = 'Imp Hr', expHr = 'Exp Hr', cfHr = "CF Hr"): # if no values are passed, use defaults
        self.path = ExcelPath
...
# =============================================================================
    ...
    def Rd_Imports(self):
        ...
        # lookup EC IDs
        ecID = self.LookupEC(Imp.index.tolist(), "Import energy carrier lookup error") # energy carriers which can be imported by model
        Imp.index = ecID
        self.ImportData = Imp
    ...
    def Rd_Exports(self):
        ...
        # lookup EC IDs
        ecID = self.LookupEC(Exp.index.tolist(), "Export energy carrier lookup error") # energy carriers which can be exported by model
        Exp.index = ecID
        self.ExportData = Exp
...
#        self.DemandEC = DemEC # demand EC data
        ...
        for i, vali in enumerate(self.inShr): # for each technology
            if len(self.inShr[vali]) > 1: # if fixed input shares are given (i.e., multiple input shares values are given for a technology)
                if len(self.inEC[vali]) != len(self.inShr[vali]): # check to ensure number of input ECs matches number of fixed input shares provided
                    raise ValueError("Mismatch between number of input energy carriers and fixed input shares; check input spreadsheet") # if not, raise error
            if len(self.outShr[vali]) > 1: # if fixed output shares are given (i.e., multiple output shares values are given for a technology)
                if len(self.outEC[vali]) != len(self.outShr[vali]): # check to ensure number of output ECs matches number of fixed output shares provided
                    raise ValueError("Mismatch between number of output energy carriers and fixed output shares; check input spreadsheet") # if not, raise error
    ...
    def Rd_Technologies(self):
        ...
        self.instCapTech = self.TechData.loc["Capacity (kW)"] # pre-installed capcities; data is the same for all hubs; keep NaN flag
        ...
        self.pldict = dict(zip(range(1,self.nTech+1), np.array(self.TechData.loc["Minimum load (%)"])/100)) # create dictionary with 1:N and part load efficiency values
        self.techPL = [k for (k,v) in self.pldict.items() if v > 0] # select partload tech IDs where partload efficiency is greater than 0  
    ...
    def Rd_Storage(self):
        ...
        self.instCapStg = self.StorageData.loc["Capacity (kWh)"] # pre-installed capcities; data is the same for all hubs; need to keep NaN flag
...
#        self.minCapStg = self.minCapStg.fillna(0)
    ...
    def Rd_EimpHr(self):            
        ...
        if self.impHr.empty is False: # if data is given
            ...
            impHrECid = self.LookupEC(impHrEC, "Hourly priced import energy carrier label not recognized. Check hourly import price input spreadsheet.")
    ...
    def Rd_EexpHr(self):            
        ...
        if self.expHr.empty is False: # if data is given
            ...
            expHrECid = self.LookupEC(expHrEC, "Hourly priced import energy carrier label not recognized. Check hourly import price input spreadsheet.")
        ...
        if self.maxCapStg.empty is False: # or self.minCapStg
            ...
            for i in range(self.nStg):
                for k in range(self.nEC):
                    for j in range(self.nHub):
                        #check if technology is present in each hub
                        if isinstance(self.hubStg[i],float) or isinstance(self.hubStg[i],int):  # present in only one hub
                            if j+1==self.hubStg[i] and k+1 == self.stgE[i]: #if hub j exists for tech i, AND the storage energy == energy carrier for tech i, then max cap is assigned; python starts with 0, Pyomo with 1
                                dmax[j,k,i] = self.maxCapStg.iloc[i]
                                dmin[j,k,i] = self.minCapStg.iloc[i]
                                
                        elif str(j+1) in list(self.hubStg[i]) and k+1 == self.stgE[i]: #python starts with 0, Pyomo with 1; if hub j exists for tech i (in a list of multiple hubs), AND the storage energy == energy carrier for tech i, then max cap is assigned;
                            dmax[j,k,i] = self.maxCapStg.iloc[i]
                            dmin[j,k,i] = self.minCapStg.iloc[i]
                    ...
                    for l in range(self.nEC):
                        ...
                        if self.node1.iloc[i] == hub_i and self.node2.iloc[i] == hub_j and self.netE[i] == EC and hub_i != hub_j:
                            ...
                            if np.isnan(self.instCapNet.iloc[i]) == False: # if installed capacity exists
                                model.CapNet[linkID, hub_i, hub_j, EC].fix(self.instCapNet.iloc[i]) # set capacity to self.instCapNet.iloc[i]
